<template>
  <div v-show="isShown">
    <right-side-bar-component
      ...
      init-event-name="open-bank-page"
    ...
    >
      ...
      <div>
        ...
        <item-delete-dialog-component v-if="isDeleteMode"
          :item="currentItem"
          item-name="Bank"
          store-action-name="Banks/deleteItem"
          archive-action-name="Banks/archiveItem"
          make-active-action-name="Banks/activateItem"
...
export default {
  ...
  computed: {
    headerName() {
      ...
      return this.isNewMode ? 'New Bank' : `${this.name}`;
    },
  ...
  data() {
    return {
      checkAssociationActionName: 'Banks/checkAssociation',
    };
  },
  ...
  methods: {
    ...
    async saveItem() {
      const result = await this.$store.dispatch('Banks/addData', this.form);
      return result;
    },

    async updateItem() {
      const result = await this.$store.dispatch('Banks/updateData', this.form);
      return result;
    },
  },
};
